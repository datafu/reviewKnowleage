...
+ (QMUIButton *)generateDarkFilledButtonWithW_H:(CGFloat)Width hight:(CGFloat)hight {
    QMUIButton *button = [[QMUIButton alloc] qmui_initWithSize:CGSizeMake(Width, hight)];
...
/*创建一个倒计时按钮*/
+ (FSPCountdownButton *)generateCountdownButton:(CGFloat)Width hight:(CGFloat)hight {
    //FSPCountdownButton
    FSPCountdownButton *button = [[FSPCountdownButton alloc] qmui_initWithSize:CGSizeMake(Width, hight)];
    button.adjustsButtonWhenHighlighted = YES;
...
+ (QMUIButton *)generateDarkFilledButton {
  return   [self generateDarkFilledButtonWithW_H:200 hight:40];
}
