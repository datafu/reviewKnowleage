...
- (void)applyConfigurationTemplate {
    ...
    QMUICMI.placeholderColor = UIColorMakeWithRGBA(255,255,255,.7);                      // UIColorPlaceholder，全局的输入框的 placeholder 颜色，默认用于 QMUITextField、QMUITextView，不影响系统 UIKit 的输入框
...
- (BOOL)shouldApplyTemplateAutomatically {
    BOOL result = ![QDThemeManager sharedInstance].currentTheme || [[[NSUserDefaults standardUserDefaults] stringForKey:QDSelectedThemeClassName] isEqualToString:NSStringFromClass(self.class)];
    if (result) {
...
- (UIColor *)themeTintColor {
    return  MacroColor_80bfff_alpha(1);
...
- (UIFont*)themeTintFont {
    return   UIFontMake(kTextSize);
}


- (UIFont*)themeButtonFont {
    return   UIFontMake(kButtonTextSize);
}
@end
